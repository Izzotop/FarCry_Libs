 ...
 */
#define INPUT_BYTE(cinfo, V, action) MAKESTMT(MAKE_BYTE_AVAIL(cinfo, action); V = GETJOCTET(*next_input_byte++);)

/* As above, but read two bytes interpreted as an unsigned 16-bit integer.
 * V should be declared unsigned int or perhaps INT32.
 */
#define INPUT_2BYTES(cinfo, V, action)                                                                                                                                             \
    MAKESTMT(MAKE_BYTE_AVAIL(cinfo, action); V = ((unsigned int)GETJOCTET(*next_input_byte++)) << 8; MAKE_BYTE_AVAIL(cinfo, action); V += GETJOCTET(*next_input_byte++);)
...
/* Process a DHT marker */
{
    ...
    while (length > 0) {
        ...
        if (*htblptr == NULL)
            *htblptr = my_jpeg_alloc_huff_table((j_common_ptr)cinfo);
...
/* Process a DQT marker */
{
    ...
    while (length > 0) {
        ...
        if (cinfo->quant_tbl_ptrs[n] == NULL)
            cinfo->quant_tbl_ptrs[n] = my_jpeg_alloc_quant_table((j_common_ptr)cinfo);
...
GLOBAL boolean my_jpeg_resync_to_restart(j_decompress_ptr cinfo, int desired) {
...
GLOBAL void jinit_marker_reader(j_decompress_ptr cinfo) {
    ...
    cinfo->marker = (struct jpeg_marker_reader*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, SIZEOF(struct jpeg_marker_reader));
