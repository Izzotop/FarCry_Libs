...
/* Allocate a "small" object */
{
    ...
    if (hdr_ptr == NULL) {
        ...
        for (;;) {
            hdr_ptr = (small_pool_ptr)my_jpeg_get_small(cinfo, min_request + slop);
            if (hdr_ptr != NULL)
                break;
            slop /= 2;
...
/* Allocate a "large" object */
{
    ...
    hdr_ptr = (large_pool_ptr)my_jpeg_get_large(cinfo, sizeofobject + SIZEOF(large_pool_hdr));
    if (hdr_ptr == NULL)
...
/* Allocate the in-memory buffers for any unrealized virtual arrays */
{
    ...
    avail_mem = my_jpeg_mem_available(cinfo, space_per_minheight, maximum_space, mem->total_space_allocated);
    ...
    for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
        if (sptr->mem_buffer == NULL) { /* if not realized yet */
            ...
            if (minheights <= max_minheights) {
                ...
                my_jpeg_open_backing_store(cinfo, &sptr->b_s_info, (long)sptr->rows_in_array * (long)sptr->samplesperrow * (long)SIZEOF(JSAMPLE));
                sptr->b_s_open = TRUE;
            }
    ...
    for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
        if (bptr->mem_buffer == NULL) { /* if not realized yet */
            ...
            if (minheights <= max_minheights) {
                ...
                my_jpeg_open_backing_store(cinfo, &bptr->b_s_info, (long)bptr->rows_in_array * (long)bptr->blocksperrow * (long)SIZEOF(JBLOCK));
                bptr->b_s_open = TRUE;
            }
...
METHODDEF void free_pool(j_common_ptr cinfo, int pool_id) {
    ...
    while (lhdr_ptr != NULL) {
        ...
        my_jpeg_free_large(cinfo, (void FAR*)lhdr_ptr, space_freed);
    ...
    while (shdr_ptr != NULL) {
        ...
        my_jpeg_free_small(cinfo, (void*)shdr_ptr, space_freed);
        mem->total_space_allocated -= space_freed;
...
METHODDEF void self_destruct(j_common_ptr cinfo) {
    ...
    my_jpeg_free_small(cinfo, (void*)cinfo->mem, SIZEOF(my_memory_mgr));
    cinfo->mem = NULL; /* ensures I will be called only once */

    my_jpeg_mem_term(cinfo); /* system-dependent cleanup */
}
...
GLOBAL void jinit_memory_mgr(j_common_ptr cinfo) {
    ...
    max_to_use = my_jpeg_mem_init(cinfo); /* system-dependent initialization */

    /* Attempt to allocate memory manager's control block */
    mem = (my_mem_ptr)my_jpeg_get_small(cinfo, SIZEOF(my_memory_mgr));
