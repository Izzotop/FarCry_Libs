...
METHODDEF int consume_data(j_decompress_ptr cinfo) {
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        compptr = cinfo->cur_comp_info[ci];
        buffer[ci] = (*cinfo->mem->access_virt_barray)((j_common_ptr)cinfo, coef->whole_image[compptr->component_index], cinfo->input_iMCU_row * compptr->v_samp_factor,
...
METHODDEF int decompress_data(j_decompress_ptr cinfo, JSAMPIMAGE output_buf) {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        buffer = (*cinfo->mem->access_virt_barray)((j_common_ptr)cinfo, coef->whole_image[ci], cinfo->output_iMCU_row * compptr->v_samp_factor, (JDIMENSION)compptr->v_samp_factor,
                                                   FALSE);
...
METHODDEF int decompress_smooth_data(j_decompress_ptr cinfo, JSAMPIMAGE output_buf) {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        if (cinfo->output_iMCU_row > 0) {
            ...
            buffer = (*cinfo->mem->access_virt_barray)((j_common_ptr)cinfo, coef->whole_image[ci], (cinfo->output_iMCU_row - 1) * compptr->v_samp_factor, (JDIMENSION)access_rows,
                                                       FALSE);
...
#endif
            coef->whole_image[ci] =
                (*cinfo->mem->request_virt_barray)((j_common_ptr)cinfo, JPOOL_IMAGE, TRUE, (JDIMENSION)jround_up((long)compptr->width_in_blocks, (long)compptr->h_samp_factor),
