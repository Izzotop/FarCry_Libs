...
#define ASSIGN_STATE(dest, src)                                                                                                                                                    \
    ((dest).last_dc_val[0] = (src).last_dc_val[0], (dest).last_dc_val[1] = (src).last_dc_val[1], (dest).last_dc_val[2] = (src).last_dc_val[2],                                     \
...
METHODDEF void start_pass_huff_decoder(j_decompress_ptr cinfo) {
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        my_jpeg_make_d_derived_tbl(cinfo, cinfo->dc_huff_tbl_ptrs[dctbl], &entropy->dc_derived_tbls[dctbl]);
        my_jpeg_make_d_derived_tbl(cinfo, cinfo->ac_huff_tbl_ptrs[actbl], &entropy->ac_derived_tbls[actbl]);
...
GLOBAL void my_jpeg_make_d_derived_tbl(j_decompress_ptr cinfo, JHUFF_TBL* htbl, d_derived_tbl** pdtbl) {
    d_derived_tbl* dtbl;
    int p, i, l, si;
    int lookbits, ctr;
    char huffsize[257];
...
GLOBAL boolean my_jpeg_fill_bit_buffer(bitread_working_state* state, register bit_buf_type get_buffer, register int bits_left, int nbits)
...
GLOBAL int my_jpeg_huff_decode(bitread_working_state* state, register bit_buf_type get_buffer, register int bits_left, d_derived_tbl* htbl, int min_bits) {
