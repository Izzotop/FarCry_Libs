...
void GeneralCombinersStruct::Validate(int numConsts, ConstColorStruct* pcc) {
    ...
    if (localConsts > 0)
        if (nullptr == glCombinerStageParameterfvNV)
...
void GeneralFunctionStruct::Validate(int stage, int portion) {
    ...
    if (numOps > 1 && op[0].reg[0].reg.bits.name == op[1].reg[0].reg.bits.name && GL_DISCARD_NV != op[0].reg[0].reg.bits.name)
        errors.set("writing to same register twice");
    if (numOps > 2 && (op[0].reg[0].reg.bits.name == op[2].reg[0].reg.bits.name || op[1].reg[0].reg.bits.name == op[2].reg[0].reg.bits.name) &&
...
void GeneralFunctionStruct::Invoke(int stage, int portion, BiasScaleEnum bs) {
    ...
    if (chan == RCP_NONE)
        chan = portion;
    glCombinerInputNV(GL_COMBINER0_NV + stage, portionEnum, GL_VARIABLE_A_NV, op[0].reg[1].reg.bits.name, op[0].reg[1].map, MAP_CHANNEL(chan));

    chan = op[0].reg[2].reg.bits.channel;
    if (chan == RCP_NONE)
        chan = portion;
    glCombinerInputNV(GL_COMBINER0_NV + stage, portionEnum, GL_VARIABLE_B_NV, op[0].reg[2].reg.bits.name, op[0].reg[2].map, MAP_CHANNEL(chan));

    chan = op[1].reg[1].reg.bits.channel;
    if (chan == RCP_NONE)
        chan = portion;
    glCombinerInputNV(GL_COMBINER0_NV + stage, portionEnum, GL_VARIABLE_C_NV, op[1].reg[1].reg.bits.name, op[1].reg[1].map, MAP_CHANNEL(chan));

    chan = op[1].reg[2].reg.bits.channel;
    if (chan == RCP_NONE)
        chan = portion;
    glCombinerInputNV(GL_COMBINER0_NV + stage, portionEnum, GL_VARIABLE_D_NV, op[1].reg[2].reg.bits.name, op[1].reg[2].map, MAP_CHANNEL(chan));

    glCombinerOutputNV(GL_COMBINER0_NV + stage, portionEnum, op[0].reg[0].reg.bits.name, op[1].reg[0].reg.bits.name, op[2].reg[0].reg.bits.name, bs.bits.scale, bs.bits.bias,
                       op[0].op, op[1].op, (op[2].op == RCP_MUX) ? true : false);
}
...
void OpStruct::Validate(int stage, int portion) {
    ...
    if (RCP_ALPHA == portion && RCP_DOT == op)
    ...
    for (i = 0; i < args; i++) {
        ...
        if (RCP_RGB == portion && RCP_BLUE == reg[i].reg.bits.channel)
            errors.set("blue register usedin rgb portion");
        if (RCP_ALPHA == portion && RCP_RGB == reg[i].reg.bits.channel)
            errors.set("rgb register used in alpha portion");
        if (i > 0 && GL_DISCARD_NV == reg[i].reg.bits.name)
