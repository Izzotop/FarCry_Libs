...
void ParseVertexProgramConstants(GLenum target, char* instring) {
    ...
    if (token != nullptr) {
        token = strtok(nullptr, lineSeparator);
    }
    ...
    while (token != nullptr) {
        ...
        if (!sscanf(token, " #%s", dummy)) { // not a comment.  Is it a constant?
                ...
                !strnicmp(token, "c[", 2)) // the first word is of the form "c[xx]", so its a constant.
            {
                ...
                if (3 != iNumValuesAssigned || toupper(c[0]) != 'C') { // error in constant directive.
                    sprintf(error, "error at line %d \n\"%s\"\n", iLineCount, token);
                    errors.set(error);
                } else if (!strnicmp(dummy, "track", 5)) { // this is a TrackMatrix directive
                    ...
                    iNumValuesAssigned = sscanf(token, " %c [ %d ] = %c%c%c%c%c ( %s %s ) ;", &c[0], &iConstID, &c[1], &c[2], &c[3], &c[4], &c[5], matrixName, matrixTransformName);
                ...
                } else // this is a constant directive
                {
                    ...
                    iNumValuesAssigned = sscanf(token, " %c [ %d ] = ( %f , %f , %f , %f ) ; ", &c, &iConstID, xyzw, xyzw + 1, xyzw + 2, xyzw + 3);

                    if (6 != iNumValuesAssigned) { // error in constant directive.
                        sprintf(error, "error at line %d \n\"%s\"\n", iLineCount, token);
                        errors.set(error);
                    } else if (iConstID < 0 || iConstID > 95) {
                        sprintf(error, "error at line %d \n\"%s\"\n\tConstant address out of range\n", iLineCount, token);
