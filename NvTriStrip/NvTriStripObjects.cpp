...
#ifdef WIN64
#pragma warning(disable : 4267) // AMD Port (conversion from size_t to int)
#endif
...
bool NvStripifier::IsMoneyFace(const NvFaceInfo& face) {
    if (FaceContainsIndex(face, 800) && FaceContainsIndex(face, 812) && FaceContainsIndex(face, 731))
        return true;
...
void NvStripifier::Stripify(const WordVec& in_indices, const int in_cacheSize, const int in_minStripLength, const unsigned short maxIndex, NvStripInfoVec& outStrips,
